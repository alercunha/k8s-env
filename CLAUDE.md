...
- Each context may have an optional `slug` (stored in the `.k8s-env` file). Slugs must be unique — `Profiles.save` rejects clashes. `ctx set`/`del`/`use` accept a slug argument to skip the interactive picker (`ctx add`/`add-remote` take `-s/--slug`, prompting when absent). `ctx slug` sets, changes, or clears the slug on the active context in place.
